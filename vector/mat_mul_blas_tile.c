...
int main(int argc, char **argv) {
 ...
 //  printf("inside main\n");
   double *a_matrix, *b_vector, *c_vector, *d_vector, *e_vector, *a_tile, *b_tile, *c_tile;
   ...
   d_vector = (double *) malloc( K*N*sizeof( double ));
   e_vector = (double *) malloc( M*N*sizeof( double ));
   init_array(a_matrix,b_vector,d_vector);
  // printMatrix(M,K,a,"A");
  // printMatrix(K,N,b,"B");
       ...
       for(i=0; i<M; i+=TILE_M)
         for(j=0; j<N; j+=TILE_N)
           for(k=0; k<K; k+=TILE_K)
           {
              ...
              k_tile = TILE_K;
              a_tile = &a_matrix[i*K+k];
              b_tile = &d_vector[k*N+j];
              c_tile = &e_vector[i*N+j];
              cblas_dgemm(CblasRowMajor,CblasNoTrans,CblasNoTrans,m,n,k_tile,alpha,a_tile,lda,b_tile,ldb,beta,c_tile,ldc);
